"""Apply 20 MiB Provider transport sharding and bounded observability.

This staging overlay is intentionally applied after the existing Provider
resilience overlay. It changes the transport target from 80 MiB to 20 MiB,
uses at most five concurrent transport shards so the Modal five-worker pool can
actually be utilized, preserves a small serialization safety ceiling, and adds
safe page-route diagnostics without logging secrets or signed URLs.
"""
...
_MARKER = "PROVIDER_TRANSPORT_SHARD_MAX_CONCURRENCY = 5"
...
def _patch_sharding() -> None:
    ...
    if _MARKER not in source:
        source = _replace_once(
            ...
            "# Materialization can differ slightly from planning serialization. Keep a\n"
            "# narrow safety ceiling while every planned shard remains <= 20 MiB.\n"
    ...
    *,
    ...
    """Run <=20 MiB transport shards with bounded per-document concurrency."""
    ...
    async def run_one(plan: ProviderInputShardPlan):
        ...
        async with semaphore:
            try:
                ...
                evidence = ProviderShardEvidence(
                    actual_plan,
                    job_id,
                    request_id,
                    outcome.raw_result,
                ...
                # Before submission, a locally materialized shard is safe to delete.
    ...
    failed_shards = 0
    for item in results:
        shard_evidence, raw_result, error, _safe, _started = item
...
'''
    source = _replace_regex_once(
        source,
        r"async def run_provider_transport_shards\(.*?(?=\ndef _delete_shard_provider_input_if_safe)",
        runner + "\n\n",
...
'''
    ...
    source = _replace_once(
        ...
        "            delivery_is_full_render=delivery_is_full_render,",
        label="provider delivery diagnostics",
    )
...
def _patch_ingestion_install() -> None:
...
def main() -> None:
    _patch_sharding()
    _patch_compat()
    _patch_ingestion_install()
    _patch_tests()
    print(
        "provider 20 MiB overlay ready: target=20971520 max=25165824 "
        "max_concurrency=5 classification_observability=enabled"
    )
