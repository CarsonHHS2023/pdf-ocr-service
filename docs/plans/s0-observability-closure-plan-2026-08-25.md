...
**Required-metric reconciliation, 2026-09-11:** [Single-PDF upload to automatic initial
...
**TXT follow-up, 2026-09-19:** [Small live worker acceptance](../reviews/s0-txt-worker-small-acceptance-2026-09-19.md)
passed on Backend `090f1f075b7ef356f3066e29848415c7db559a93`: 64,476 retained
bytes and **23.63936695 s** measured worker wall time, with Reader body and TOC
confirmed by the user. PR #49 is merged/deployed. This auxiliary result does not
change 17/19 or replace the registered TXT fixture baseline. Native collector
replay and private fixture-identity verification were not part of this live audit.
...
**2026-09-02 failure/retry small checkpoint:** [S0.3.6 small success-path acceptance](../reviews/s0-3-6-failure-retry-small-acceptance-2026-09-02.md) passed on exact Backend/runtime `7435aa3fa7ba0766d8cc2584bcacfd735c5ce74c`. The fresh one-page run has `failure_retry_counts = observed`: 14 successful Provider method-call entries (1 submit, 12 normal status polls, 1 result), zero failures/retries/cancellations, and complete start/single-scope/terminal closure. The collector left **four** required metrics `not_instrumented`; multi-scope runtime evidence was next at that checkpoint.
...
**2026-09-09 visual-generation checkpoint:** [Fresh one-page acceptance](../reviews/s0-visual-asset-generation-small-acceptance-2026-09-09.md) passed on exact Backend/runtime `a640cf07c0b3db8e0cade4950e4cc74af2ac0cfc`. The deployed collector replay observes `visual_asset_generation_seconds = 0.253639226` with one complete ordinal `0,1` pair and two generated assets/two persisted renditions. The complete 47-event snapshot has 16 of 19 required metrics observed; **three** required gaps remain. Medium is not required for this single-operation target. The next design item is the upload-to-Reader-ready boundary in Section 6.1; full upload memory and full-stage CPU remain unresolved, without a scope waiver or phase advance.
...
**Implemented; representative small and medium success-path Staging acceptance PASS (2026-09-02).** [PR #41](https://github.com/CarsonHHS2023/pdf-ocr-service/pull/41) added the Staging-only producer, durable persistence and strict collector mapping without changing retry policy. The [contract](../testing/s0-failure-retry-observability-v1.md), [small checkpoint](../reviews/s0-3-6-failure-retry-small-acceptance-2026-09-02.md) and [medium follow-up](../reviews/s0-3-6-failure-retry-medium-acceptance-2026-09-02.md) pin the separate method-call, orchestration, Provider-terminal and logical-invocation layers. Medium establishes sequential two-scope runtime aggregation; concurrent execution and real fault-injection acceptance are not claimed.
...
- Local fake-provider/fault-injection contract tests cover success, retryable failure then actual retry, cancellation/deadline before dispatch, non-retryable failure, duplicate/missing terminal evidence and per-shard attribution. Fresh small and sequentially sharded medium runs add real success-path evidence only. No additional Provider job, failure injection into Staging, upload or benchmark is authorized by this documentation update.
...
| S0 requirement | Current status after 2026-09-19 reconciliation | Closure action |
|---|---|---|
...
| visual asset generation duration | `observed` on the 2026-09-09 one-page checkpoint; `0.253639226 s` | retain the final composed enrichment-call boundary; small acceptance PASS |
...
| TXT representative baseline | small live worker control accepted at 23.63936695 s; formal registered baseline remains open | retain scoped evidence; verify private fixture identity and native collector report for formal baseline; medium needs incremental coverage |
...
6. **Next executable work:** the [scoped upload/Reader review](../reviews/s0-3-7-upload-reader-contract-review-2026-09-11.md) and [TXT source inspection](s0-txt-lifecycle-timing-plan-2026-09-11.md) are recorded. PR #48 now fixes duration containment, ambiguous Reader scope admission and upload envelope validation; it is merged and deployed on `593d65199c6e21571e0094014d41dc30269f9adc`, with [tested-artifact and exact-runtime evidence](../reviews/s0-3-7-collector-admission-review-2026-09-19.md). PR #49 is now merged/deployed and its small live worker control is accepted. Continue S0.3.7 beyond the corrected upload/Reader path: review the remaining event-family contracts, cross-family snapshot consistency, revision trust, relational candidate admission and privacy. Reproduce candidate defects before implementation; retain the remaining formal TXT baseline gates. The [attribution decision record](s0-remaining-attribution-decisions-2026-09-11.md) keeps full upload memory and preprocessing CPU unimplemented until a complete method or explicit scope decision exists. Neither more fixtures nor auxiliary-metric renaming resolves these gaps.
