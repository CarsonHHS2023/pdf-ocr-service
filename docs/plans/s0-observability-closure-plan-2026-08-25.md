...
5. **Next: upload-to-Reader-ready boundary review.** The [source-pinned proposal](../testing/s0-upload-to-reader-ready-observability-v1.md) selects the existing single-file automatic-open flow, a single browser clock and an explicitly named core-semantic-render endpoint. It specifies required same-upload/run/candidate proof and fail-closed admission without changing polling, selection or rendering behavior. The proposed endpoint is not browser-paint or binary completion. Review that scope before runtime implementation or required-metric mapping; `upload_to_reader_ready_seconds` remains `not_instrumented` and the three required gaps remain open.
