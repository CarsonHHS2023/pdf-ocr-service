...
4. **Visual asset generation timing:** the [v1 boundary](../testing/s0-visual-asset-generation-observability-v1.md) and [implementation review](../reviews/s0-visual-asset-generation-implementation-2026-09-04.md) select the final composed PDF candidate-enrichment call, excluding source read, structure refinement, SPR persistence and candidate database commit. The implementation candidate adds bounded two-event durable coverage and strict mapping without changing visual behavior. It is not Staging acceptance; the required metric remains `not_instrumented` in accepted evidence until an exact deployed eligible run passes. Canonicalization duration remains a separate containing interval and is not a substitute.
