...
3. **Preprocessing CPU attribution design — 2026-09-03:** the [proposed boundary](../testing/s0-preprocessing-cpu-attribution-v1.md) and [13 local synthetic controls per Python version](../reviews/s0-preprocessing-cpu-feasibility-2026-09-03.md) distinguish the synchronous worker's CPU component from whole-stage CPU. Current process deltas remain auxiliary; native helper coverage is not proven. Review whether to implement the explicitly named worker-thread auxiliary, with a finalized bounded lifecycle/persistence schema first. `preprocessing_cpu_seconds` stays `not_instrumented`; no runtime producer, collector mapping or acceptance is claimed.
