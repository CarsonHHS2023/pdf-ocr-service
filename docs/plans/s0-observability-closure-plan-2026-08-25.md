...
The **four** remaining `not_instrumented` rows in both fresh S0.3.6 small and medium collectors are `backend_upload_peak_memory_mb`, `preprocessing_cpu_seconds`, `visual_asset_generation_seconds`, and `upload_to_reader_ready_seconds`. These are actual collector gaps, not accepted waivers. Historical S0.3.4 seven-gap and S0.3.5 five-gap checkpoints remain valid for their original snapshots. The matrix combines explicitly scoped acceptance across revisions; it does not claim every representative case was freshly measured on one revision or every auxiliary metric is available. Although Reader metrics are also observed in the latest snapshots, this failure/retry acceptance did not perform a separate first-open/reopen Reader acceptance audit. One-page Reader acceptance, TXT ingestion timing, Reader binary access coverage and final S0 review remain open. Sequential S0.3.6 multi-scope runtime coverage is now satisfied; real nonzero-retry and concurrent-runtime coverage are not claimed.
...
2. **Upload-owned memory gap retained:** the design/local feasibility work in [Draft PR #42](https://github.com/CarsonHHS2023/pdf-ocr-service/pull/42), inspected at `0e026816e07dd393ac55d841790a60ffdc06fce2`, does not establish a complete attributable upload peak. That PR remains separate and unmerged at this checkpoint. Largest read-buffer bytes or process-lifetime RSS cannot fill `backend_upload_peak_memory_mb`; continuing CPU design grants no waiver.
3. **Preprocessing CPU attribution design — 2026-09-03:** the [proposed boundary](../testing/s0-preprocessing-cpu-attribution-v1.md) and [local controls](../reviews/s0-preprocessing-cpu-feasibility-2026-09-03.md) distinguish the synchronous worker's CPU component from whole-stage CPU. Current process deltas remain auxiliary; native helper coverage is not proven. The [protocol follow-up](../testing/s0-preprocessing-worker-cpu-events-v1.md) now defines bounded fields, registration versus actual stage entry, cancellation-safe closure and atomic publication requirements. Review the auxiliary design before implementing the actual producer/writer/validator and named CI tests. `preprocessing_cpu_seconds` stays `not_instrumented`; no runtime producer, collector mapping or acceptance is claimed.
