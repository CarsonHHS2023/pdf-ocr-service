...
request boundary through durable upload acceptance.  It is deliberately
observational: it does not change upload routing, source retention, database
semantics, background-task ownership, or response payloads.

Only the canonical multipart route receives the required upload-duration
measurement.  Resumable and direct-object upload routes have different timing
boundaries and must not be collapsed into this contract.
...
def install_s0_upload_boundary_observability(*, force: bool = False) -> bool:
    ...
    if _INSTALLED:
        return True
    if not force and not staging_processing_events_enabled():
        return False
